...
//� decommenter si gestion individuelle du temps de recherche
//	public int cycleCherche = 0;
	public int cycleVadrouille = 0;
// valeur de comportement propre � chaque agent
...
// remise � z�ro lorsque l'agent repart
...
// modification de l'�tat de l'agent
...
// d�finie mon nouvel �tat
	...
	/*
	 * agent se d�place et choisi son prochain arc. attentePlace => arret (si
	 * placeLibre) vadrouille => cherche si nbreCycle correct
	 */
	
	protected  Arc arcSuivant(){
		if (sens) // dans le sens des arcs donc � la fin
...
// je suis arriv� au bout de l'arc	
...
//retourne l'arc courant si  le bon arc mais mauvais sens.
			if (suiv.equals(cr.getArcCourant())){// je reste sur le m�me arc => demi tour
						if (sens)
							position = suiv.getNbrePlaces() - 1;
						else 
							position = 0;
						sens = !sens;
				}else if (sens){ // je suis � la fin de l'arc courant
						if (cr.getArcCourant().getFin().equals(suiv.getDebut())) {
							...
							// je suis � la fin de l'arc suivant
							position = suiv.getNbrePlaces() - 1;
							sens = false;
						} 
	...
	protected void tr_vadrouille_cherche(){
		miseAjourDemande(); // met � jour le nombre de demande selon le type d'agents.
		setState(TurtleState.cherche); // je me met en �tat de recherche
		cycleVadrouille = 0;
//		cycleCherche = 1;
	}
	/*
	* passe de l'�tat arret � l'�tat vadrouille
...
//			cycleCherche++;
	...
	/*
	 * je me suis d�plac�, je regarde mon nouvel �tat et me met � jour
	 */
	...
	switch (state) {
		...
		// je suis � la position d'une place et c'est libre
	...
	public int nbreStationnePositionBis() {
		...
		for (int k = 0; k < turtlehere.length; k++) {
			if (((TurtleNetWorkTurtle) (turtlehere[k])).state == TurtleState.arret) {
				nbrestationne++;
//				System.out.println("agent d�ja pr�sent : " + turtlehere[k].getName());
			}
		}
		return nbrestationne;
	}
